...
delimiter //
create procedure add_owner (in ip_username varchar(40), in ip_first_name 
varchar(100), in ip_last_name varchar(100), in ip_address varchar(500), in ip_birthdate 
date)
sp_main: begin
...
in ip_taxID varchar(40), in ip_hired date, in ip_employee_experience integer, in ip_salary integer)
sp_main: begin
    -- ensure new owner has a unique username
    ...
    then 
		...
		values (ip_username, ip_taxID, ip_hired, ip_employee_experience);
    end if;
...
delimiter //
create procedure add_ingredient (in ip_barcode varchar(40), in ip_iname 
varchar(100),
...
delimiter //
create procedure add_drone (in ip_id varchar(40), in ip_tag integer, in ip_fuel 
integer,
...
sp_main: begin
	...
	and ip_tag not in (select tag from drones)
    and flown_by in (select username from pilots)) then
...
delimiter //
create procedure start_funding (in ip_owner varchar(40), in ip_long_name 
varchar(40))
sp_main: begin
...
sp_main: begin
	...
	if (ip_username in (select username from work_for where ip_id = id)
    and ip_username not in (select manager from delivery_services where id = ip_id)
    and ip_username not in (select flown_by from drones)) then
...
sp_main: begin
    ...
    select swarm_tag as boss from drones where (ip_id = id and ip_tag = tag);
    if (ip_username in (select username from work_for where id = ip_id)
    and ip_id in (select id from drones where ip_tag = tag)
    and boss is null
    and ip_username not in (select manager from delivery_services)
    and ip_username in (select username from pilots)) then
		update drones set manager = ip_username where (ip_tag = tag and ip_id = id);
	end if;
...
sp_main: begin
    ...
    and ip_id in (select id from drones where tag = ip_swarm_leader_tag)
    and (select flown_by from drones where (id = ip_id and tag = ip_swarm_leader_tag)) is not null
    and (select hover from drones where (id = ip_id and tag = ip_swarm_leader_tag))
    =(select hover from drones where (id = ip_id and tag = ip_tag))) then
...
sp_main: begin
	...
	if (ip_id in (select id from drones where tag = ip_swarm_tag)
    and (select swarm_tag from drones where ip_id = id) = ip_swarm_tag) then
		update drones set flown_by = (select flown_by from drones where tag = ip_swarm_tag) where id = ip_id;
        update drones set swarm_tag = NULL where id = ip_id;
    end if;
...
of the ingredient already loaded onto the drone as applicable.  And if the 
ingredient
...
delimiter //
create procedure refuel_drone (in ip_id varchar(40), in ip_tag integer, in 
ip_more_fuel integer)
sp_main: begin
...
delimiter //
create procedure fly_drone (in ip_id varchar(40), in ip_tag integer, in 
ip_destination varchar(40))
sp_main: begin
-- ensure that the lead drone being flown is directly controlled and owned bythe service
...
at its current location.  The drone must have the desired quantity of the 
ingredient 
...
sp_main: begin
    ...
    if (ip_tag in (select tag from drones where ip = ip_id)
    and ip_tag in (select tag from payload where ip = ip_id)
    and ip_tag not in (select swarm_tag from drones where id = ip_id)) then
...
sp_main: begin
    ...
    and ip_username not in (select flown_by from drones)) then
		if (ip_username in (select username from workers)) then
			...
			delete from employees where ip_username = username;
		end if;
	end if;
end //
delimiter ;
...
packages
that can be purchased and the lowest and highest prices at which the ingredient is 
being
...
number
of unique ingredients along with the total cost and weight of those ingredients 
being
